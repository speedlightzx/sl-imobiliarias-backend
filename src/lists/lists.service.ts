...
import { leadTemperatureEnum } from '@/database/schemas/schemas';
...
export class ListsService {
  ...
  async getAllLists(userId: number) {
    const lists = await this.db
      .select({
        ...
        leads: sql<
          { name: string; temperature: typeof LeadTemperature; status: typeof LeadStatus;  id: number }[]
        >`
            COALESCE(
                json_agg(
                    json_build_object(
  ...
  async updateList(userId: number, dto: updateListDTO, listId: number) {
    ...
    const [listExists] = await this.db
      ...
      .from(schema.lists)
      .where(
        and(eq(schema.lists.id, listId), eq(schema.lists.user_id, userId)),
      );

    if (!listExists)
      throw new NotFoundException(
        `Não foi encontrada nenhuma lista com id ${listId}`,
      );
