...
export class LeadsService {
  ...
  async createLead(dto: createLeadDTO, userId: number) {
    const [listExists] = await this.db
      ...
      .from(schema.lists)
      .where(
        and(eq(schema.lists.id, dto.listId), eq(schema.lists.user_id, userId)),
      );

    if (!listExists)
      throw new NotFoundException(
        `Não foi encontrado nenhuma lista com o ID ${dto.listId}`,
      );
  ...
  async updateLead(dto: updateLeadDTO, userId: number, leadId: number) {
    if (!dto.name && !dto.status && !dto.listId && !dto.temperature)
      throw new BadRequestException(
        'Você precisa enviar ao menos um campo para editar.',
      );
    ...
    const [leadExists] = await this.db
      ...
      .where(and(eq(schema.leads.id, leadId), eq(schema.lists.user_id, userId)))
      .limit(1);

    if (!leadExists)
      throw new NotFoundException(
        `Não foi possível encontrar o lead de ID ${leadId}.`,
      );
    ...
    if (dto.listId) {
      ...
      if (!listExists)
        throw new NotFoundException(
          `Não foi possível encontrar a lista de ID ${dto.listId}.`,
        );
    }
  ...
  async deleteLead(userId: number, leadId: number) {
    const [leadExists] = await this.db
      ...
      .where(
        and(eq(schema.leads.id, leadId), eq(schema.lists.user_id, userId)),
      );

    if (!leadExists)
      throw new NotFoundException(
        `Não foi encontrado nenhum lead com o ID ${leadId}`,
      );
