...
export class ChatbotGateway {
  ...
  async handleMessage(
    ...
    try {
      const res = await fetch(`${process.env.LLM_MICROSERVICE_URL}/sendMessage`, {
        method: 'POST',
      ...
      client.emit('response', 'Algum erro aconteceu durante a comunicação com a IA. Tente novamente mais tarde.')
      return
    } finally {
  ...
  async handleConnection(client:Socket) {
    ...
    if(!token || !await this.authService.isValidToken(token)) client.disconnect()
  }
}
